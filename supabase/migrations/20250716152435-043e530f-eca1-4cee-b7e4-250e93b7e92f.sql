...
SELECT 
  'MAT-001', 'กระดาษ A4', c.id, 50, 20, 100, 'รีม', 90.00, '8850999320101', 'บริษัท ABC จำกัด', '2024-01-10'
FROM category_ids c WHERE c.name = 'เครื่องเขียน'
UNION ALL
SELECT 
  'MAT-002', 'ปากกาลูกลื่น', c.id, 200, 50, 300, 'ด้าม', 15.00, '8850999320102', 'บริษัท XYZ จำกัด', '2024-01-08'
FROM category_ids c WHERE c.name = 'เครื่องเขียน'
UNION ALL
SELECT 
  'MAT-003', 'แฟ้ม', c.id, 15, 30, 80, 'เล่ม', 25.00, '8850999320103', 'บริษัท ABC จำกัด', '2024-01-05'
FROM category_ids c WHERE c.name = 'เครื่องเขียน'
UNION ALL
SELECT 
  'MAT-004', 'เมาส์ไร้สาย', c.id, 8, 10, 25, 'ชิ้น', 450.00, '8850999320104', 'บริษัท Tech จำกัด', '2024-01-03'
FROM category_ids c WHERE c.name = 'อุปกรณ์ IT'
UNION ALL
SELECT 
  'MAT-005', 'คีย์บอร์ด', c.id, 12, 5, 20, 'ชิ้น', 650.00, '8850999320105', 'บริษัท Tech จำกัด', '2024-01-01'
FROM category_ids c WHERE c.name = 'อุปกรณ์ IT'
UNION ALL
SELECT 
  'MAT-006', 'กาว', c.id, 25, 15, 50, 'หลอด', 8.00, '8850999320106', 'บริษัท ABC จำกัด', '2024-01-12'
FROM category_ids c WHERE c.name = 'เครื่องเขียน';